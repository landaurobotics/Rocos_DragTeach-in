...
#pragma endregion
        ...
        if (outputFile.is_open())
        {
            ...
            const double peak_current = 20.0; // 20A
            const double torque_constant = 0.14; // torque constant 0.14 Nm/A
            const double ratio = 120.0;
            ...
            while (isRuning)
            {
                ...
                for (int i = 0; i < _joint_num; ++i)
                {
                    ...
                    joint_real_torques(i) = getJointTorque(i) / 1000.0 * peak_current * torque_constant * ratio;

                    std::cout << "tor: " << joint_real_torques(i) << "\t";
                ...
                for (size_t i = 0; i < _joint_num; ++i)
                {
                    ...
                    error = joint_torques(i)- joint_tor[i];
                    d_error = error - error_prev;
                    double low_torque = joint_torques(i) + P*error - D*d_error;
                    std::cout << "low tor: " << -low_torque;
                    joint_cmd_torques(i) = -low_torque / ratio / torque_constant / peak_current * 1000 * 1.3;
                    setJointTorque(i, joint_cmd_torques(i));
