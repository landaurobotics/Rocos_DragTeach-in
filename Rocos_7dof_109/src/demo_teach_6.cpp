...
namespace rocos
{
    ...
    double segmentedSigmoid(double x, double scale = 8, double shift = 23.0, double amplitude = 40.0)
    {
        //46->5,23,30
        //5->6.5,26,30
        double y;
...
#pragma region //
        ...
        double com_fext = 0.0;
        double pose = getJointPosition(5);
        double a_6 = 42.72;
        double b_6 = -0.8113;
        double a_4 = 90.19;
        double b_4 = -1.743;
        ...
        while (isRuning)
        {
            ...
            if (j > 1000)
            {
                ...
                // {
                std::cout << "joint_tor: " << joint_tor[5] * a_6 + b_6<< "\t";
                std::cout << "theory: " << joint_torques(5) << "\t";
                fext = joint_tor[5] * a_6 + b_6- joint_torques(5);
                std::cout << "fext: " << fext << "\t";
                ...
                q_target1(5) = pose;
                if (pose< -90.0 * M_PI / 180.0 || pose> 100.0 * M_PI / 180.0)
                {
                    std::cerr << "Joint angle out of bounds" << std::endl;
                    std::exit(1);
                }
                std::cout << "com pose: " << q_target1(1) << std::endl;
                ;
                last_pose = pose;
                // }
                servoJ(q_target1);
